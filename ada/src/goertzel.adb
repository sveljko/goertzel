...
is
   ...
   function Calc_Koef(F, Fs: Value) return Value 
   with
       Pre => (Fs < 100_000.0) and (F < Fs / 2.0) and (Fs > 0.0)
   is
   begin
      return 2.0 * Cos(2.0 * Ada.Numerics.Pi * F / Fs);
   end Calc_Koef;
   ...
   function Make(F, Fs: Value) return Filter is
      Rslt: Filter;
   begin
      Rslt.F := F;
      Rslt.Fs := Fs;
      Rslt.Koef := Calc_Koef(F, Fs);
      Reset(Rslt.Vn);
      return Rslt;
   end Make;
   ...
   function DBm(Power: Value) return Value is (10.0 * Log(2.0 * Power * 1000.0 / 600.0));
